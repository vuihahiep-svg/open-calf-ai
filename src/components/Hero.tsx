...
const SNIPPET = `git clone https://github.com/a20-ai-thuc-chien/A20-App-097.git
cd A20-App-097
cp .env.example .env
...
export function Hero() {
  ...
  return (
    <section className="relative overflow-hidden">
      ...
      <div
        className="absolute left-1/2 top-0 -z-10 h-[500px] w-[800px] -translate-x-1/2 rounded-full bg-primary/10 blur-3xl"
        aria-hidden
      />
      ...
      <div className="mx-auto max-w-7xl px-6 pb-24 pt-20 lg:pt-28">
        <div className="mx-auto max-w-4xl text-center">
          ...
          <p className="mx-auto mt-6 max-w-2xl text-balance text-lg text-muted-foreground">
            Open-source{" "}
            <strong className="font-semibold text-foreground">AI Agents platform</strong> —
            RAG-powered chat, voice, multi-channel support, and{" "}
            <strong className="font-semibold text-foreground">lead extraction</strong> to the
            dashboard.
          </p>

          <p className="mt-3 font-mono text-xs text-muted-foreground">
            MIT · Docker-friendly · TypeScript — agents, widget, optional voice (LiveKit), headless
            API.
          </p>
          ...
          <div className="overflow-hidden rounded-xl border border-border bg-surface/80 shadow-2xl backdrop-blur">
            <div className="flex items-center justify-between border-b border-border bg-surface-elevated/60 px-4 py-2.5">
              <div className="flex items-center gap-2">
                ...
                <span className="h-2.5 w-2.5 rounded-full bg-foreground/15" />
                <span className="h-2.5 w-2.5 rounded-full bg-foreground/15" />
                <span className="ml-3 font-mono text-xs text-muted-foreground">~/A20-App-097</span>
              </div>
              <button
                onClick={copy}
              ...
              <code>
                {SNIPPET.split("\n").map((line, i) => (
                  <div key={i} className="flex">
                    ...
                    <span
                      className={line.startsWith("#") ? "text-muted-foreground" : "text-foreground"}
                    >
                      {line.startsWith("#") ? (
                        line
                      ) : line.startsWith("git") ||
